...
import { Layout, Menu, Avatar, Dropdown, Button, Drawer, Spin, Space, Grid, Badge, App } from "antd";
import {
...
const TechnicianLayout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchUnreadCount = async () => {
      ...
      const { data, error } = await supabase
        .from("notifications")
        .select("*")
        .eq("technician", true) // Assuming you're filtering for technician-specific notifications
        .eq("status", "unread")
  ...
  console.log("Unread Count:", unreadCount); // Debugging line
